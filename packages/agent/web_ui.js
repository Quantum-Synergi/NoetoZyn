...
const WALLET_BUNDLE_PATH = fileURLToPath(new URL('./public/wallet-connect.js', import.meta.url));
...
<section class="nz-panel nz-panel-neutral"><h2 class="nz-panel-title">Optional: connect your Solana wallet</h2><p class="nz-panel-sub">Wallet connection is not required. This display is identity-only and never signs, uploads, or changes the server-side guardian.</p><button class="nz-btn" id="wallet-button">Connect wallet</button><select class="nz-btn" id="wallet-chooser" aria-label="Choose a detected wallet" hidden></select><span class="muted" id="wallet-state" aria-live="polite">Disconnected</span><div id="wallet-connected" hidden><strong id="wallet-name"></strong><span class="nz-mono" id="wallet-address"></span><button class="nz-btn" id="copy-address" type="button">Copy address</button><button class="nz-btn" id="disconnect-wallet" type="button">Disconnect</button></div></section>
...
async function route(request, response) {
  ...
  if (request.method === 'GET' && path === '/assets/wallet-connect.js') {
    try {
      response.writeHead(200, { 'Content-Type': 'text/javascript; charset=utf-8', 'Cache-Control': 'public, max-age=300' });
      response.end(fs.readFileSync(WALLET_BUNDLE_PATH));
    } catch {
      jsonResponse(response, 404, { error: 'Wallet bundle not found' });
    }
    return;
  }
